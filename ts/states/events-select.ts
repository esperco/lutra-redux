...
export type ToggleEventAction = ClearAction|ToggleAction;
...
export function reduceEventToggling<S extends EventsSelectState & EventsState> (
  ...
  if (isToggle(action)) {
    if (action.value) {
      selectedEvents[action.eventId] = true;
    } else {
      delete selectedEvents[action.eventId];

      // Delete all recurrences as well
      let eventMap = state.groupEvents[action.groupId] || {};
      let event = eventMap[action.eventId];
...
function isToggle(action: ToggleEventAction): action is ToggleAction {
  return !!(<any> action).eventId;
}
